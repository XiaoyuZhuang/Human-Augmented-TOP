function clickCallback_add_void(src, event)
...
while true
    ...
    L = 2 * semiAxes(1);t0 = semiAxes(2);t = [t0, t0, t0];
    sin_theta = sin(theta);

    % Update void component parameters
    new_component = [x0, y0, L/2, t, sin_theta];N = N + 1;
    ...
    disp('Void component added successfully!')
    ...
    if strcmp(answer, 'No')
        close(newFig);
        contourf(reshape(x, M), reshape(y, M), Phi_max, [0, 0]);axis equal;axis([0 DW 0 DH]);pause(1e-6);
        break;
    end
end