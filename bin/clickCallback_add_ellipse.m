function clickCallback_add_ellipse(src, event)
% Function to handle click events and allow the user to draw ellipses

global_declare; % Declare global variables

% Create a new figure and axes for drawing
newFig = figure;ax = axes(newFig);plot_result; % Initial plot

while true
    % Wait for the user to draw an ellipse
    h = drawellipse(ax, 'Color', 'red');wait(h);
    ...
    xy00_his{end + 1} = xy00;xval = xy00;xold1 = xy00;xold2 = xy00;
    ...
    disp('Ellipse added successfully!');
    Phi{N} = tPhi(xy00(Var_num*N - Var_num + 1:Var_num*N), LSgrid.x, LSgrid.y, 2);
    fixed_node{end + 1, 1} = 1;fixed_node{end, 2} = Phi{N};

    plot_result;

    % Ask user if they want to continue drawing
    answer = questdlg('Would you like to draw another ellipse?', 'Continue Drawing', 'Yes', 'No', 'Yes');
    if strcmp(answer, 'No')
        close(newFig);
        contourf(reshape(x, M), reshape(y, M), Phi_max, [0, 0]);axis equal;
        axis([0 DW 0 DH]);pause(1e-6);
        break;
    end
end